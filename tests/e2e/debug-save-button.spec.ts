...
test('debug save button', async ({ page }) => {
  ...
  const allErrorElements = await page.locator('[class*="error"], [class*="red"], [class*="invalid"]').count();
  ...
  const yamlErrorElementsAfter = await page.locator('[class*="red-50"], [class*="red-600"]').count();
