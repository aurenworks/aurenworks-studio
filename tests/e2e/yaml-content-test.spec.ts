...
test('yaml content test', async ({ page }) => {
  ...
  const errorElements = await page.locator('[class*="error"], [class*="red"]').count();
  ...
  const validationErrors = await page.locator('[class*="validation"], [class*="invalid"]').count();
  console.log('Validation errors found:', validationErrors);
  
  // Take a screenshot for debugging
  await page.screenshot({ path: 'test-results/yaml-content-test.png' });
});
